...
export default function Story () {
    ...
    const StoryCreator = async (): Promise<void> => {
        ...
        try {
            ...
            // Parse the response for FA and EN parts
            const text = response.text || "No story generated.";
            setStory(text); // still keep the raw output
            // Try to extract Persian and English parts (no 's' flag)
            const faMatch = text.match(/Persian:?\s*\[FA\]([\s\S]*?)English:?/);
            const enMatch = text.match(/English:?\s*\[EN\]([\s\S]*)/);
            setStoryFa(faMatch ? faMatch[1].trim() : "");
            setStoryEn(enMatch ? enMatch[1].trim() : "");
            setShowStory(true);
        } catch (err: any) {
